...
const RegisterConfirm = () => {
 ...
 const {mutator, loading} = useMutationAPI({
  endpoint: "/auth/register-confirm",
  clearError: 5000,
 ...
 const handleRegister = async (event: FormEvent<HTMLFormElement>) => {
  ...
  await mutator({token,code:Code,password:Password});
 };
 ...
 return (
  <main className="h-screen w-full grid place-items-center bg-white">
   <form onSubmit={handleRegister}
    ...
    <div>
     <h1 className="text-2xl font-extrabold mb-1.5 tracking-[0.4px]">Register Confirm</h1>
     <p className="text-gray-600 text-sm">
      You are one step away from creating account
     </p>
    </div>
    <input
     className={input}
